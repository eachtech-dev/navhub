...
9. Для экспорта статики и деплоя её в surge существует 3 команды:
   * `yarn deploy` - деплой на случайно сгенерированный адрес, например `waggish-trees.surge.sh`. Потребуется подтверждение в терминале
   * `yarn deploy:preview` - деплой на адрес `navhub-{uuid}.surge.sh`, где {uuid} - случайно сгенерированная последовательность чисел и символов. Подходит для быстрой проверки результата, не требует подтверждения
   * `yarn deploy:production` - деплой на адрес https://eachtech-navhub.surge.sh, подходит для деплоя и демонстрации окончательного результата, не требует подтверждения
   1. *Примечание: если адрес уже был занят кем-то, кто не добавил вас в список доступа, вы получите ошибку при деплое. Конкретно с таким можно будет столкнуться в 1 и 3 командах*
